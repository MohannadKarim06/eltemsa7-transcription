...
class AudioProcessor:
    ...
    def __init__(self, target_sr: int = 16000):
        ...
        logger.info(f"AudioProcessor initialized with target sample rate: {self.target_sr}")
    ...
    def preprocess_audio(self, input_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                if duration < 1.0: # Adjusted threshold to be more robust
                    logger.warning(f"Audio file too short ({duration:.2f}s), skipping")
                    return False

            except Exception as e:
                logger.warning(f"Could not get audio duration for {input_path}: {e}")
                # Continue processing even if duration check fails, ffmpeg might still work
            ...
            logger.error(f"Audio preprocessing failed for {input_path}: {e}")
            return False